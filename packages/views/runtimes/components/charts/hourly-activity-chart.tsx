...
export function HourlyActivityChart({ runtimeId }: { runtimeId: string }) {
  ...
  return (
    <div className="rounded-lg border p-4">
      <h4 className="text-xs font-medium text-muted-foreground mb-3">{t("runtimes.charts.hourlyDistribution")}</h4>
      {loading ? (
        <div className="flex h-[140px] items-center justify-center text-xs text-muted-foreground">
          {t("runtimes.charts.loading")}
        </div>
      ) : !hasData ? (
        <div className="flex h-[140px] flex-col items-center justify-center">
          <BarChart3 className="h-5 w-5 text-muted-foreground/40" />
          <p className="mt-2 text-xs text-muted-foreground">{t("runtimes.charts.noTaskData")}</p>
        </div>
      ) : (
