...
export function UsageSection({ runtimeId }: { runtimeId: string }) {
  ...
  if (usage.length === 0) {
    return (
      <div className="flex flex-col items-center rounded-lg border border-dashed py-6">
        ...
        <p className="mt-2 text-xs text-muted-foreground">{t("runtimes.usage.empty")}</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Summary cards */}
      <div className="grid grid-cols-4 gap-3">
        <TokenCard label={t("runtimes.usage.input")} value={formatTokens(totals.input)} />
        <TokenCard label={t("runtimes.usage.output")} value={formatTokens(totals.output)} />
        <TokenCard label={t("runtimes.usage.cacheRead")} value={formatTokens(totals.cacheRead)} />
        <TokenCard label={t("runtimes.usage.cacheWrite")} value={formatTokens(totals.cacheWrite)} />
      </div>

      {totals.cost > 0 && (
        <div className="rounded-lg border bg-muted/30 px-3 py-2">
          <span className="text-xs text-muted-foreground">
            {t("runtimes.usage.estimatedCost", { days })}
          </span>
      ...
      <div className="rounded-lg border">
        <div className="grid grid-cols-[100px_1fr_80px_80px_80px_80px] gap-2 border-b px-3 py-2 text-xs font-medium text-muted-foreground">
          <div>{t("runtimes.usage.tableDate")}</div>
          <div>{t("runtimes.usage.tableModel")}</div>
          <div className="text-right">{t("runtimes.usage.input")}</div>
          <div className="text-right">{t("runtimes.usage.output")}</div>
          <div className="text-right">{t("runtimes.usage.tableCacheR")}</div>
          <div className="text-right">{t("runtimes.usage.tableCacheW")}</div>
        </div>
