...
export function RuntimeDetail({ runtime }: { runtime: AgentRuntime }) {
  ...
  const handleDelete = () => {
    deleteMutation.mutate(runtime.id, {
      onSuccess: () => {
        toast.success(t("runtimes.detail.deletedToast"));
        setDeleteOpen(false);
      },
      onError: (e) => {
        toast.error(e instanceof Error ? e.message : t("runtimes.detail.deleteFailed"));
      },
    });
  };
    ...
    <div className="flex h-full flex-col">
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6">
        ...
        <div className="grid grid-cols-2 gap-4">
          <InfoField label={t("runtimes.detail.runtimeMode")} value={runtime.runtime_mode} />
          <InfoField label={t("runtimes.detail.provider")} value={runtime.provider} />
          <InfoField label={t("runtimes.detail.status")} value={runtime.status} />
          <InfoField
          ...
          {ownerMember && (
            <div>
              <div className="text-xs text-muted-foreground mb-1">{t("runtimes.detail.owner")}</div>
              <div className="flex items-center gap-2">
                <ActorAvatar
                  actorType="member"
          ...
          {runtime.device_info && (
            <InfoField label={t("runtimes.detail.device")} value={runtime.device_info} />
          )}
          {runtime.daemon_id && (
            <InfoField label={t("runtimes.detail.daemonId")} value={runtime.daemon_id} mono />
          )}
        </div>
        ...
        <div>
          <h3 className="text-xs font-medium text-muted-foreground mb-3">
            {t("runtimes.detail.connectionTest")}
          </h3>
        ...
        <div className="grid grid-cols-2 gap-4 border-t pt-4">
          <InfoField
            label={t("runtimes.detail.created")}
            value={new Date(runtime.created_at).toLocaleString()}
          />
          <InfoField
            label={t("runtimes.detail.updated")}
      ...
      <AlertDialog open={deleteOpen} onOpenChange={(v) => { if (!v) setDeleteOpen(false); }}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{t("runtimes.detail.deleteTitle")}</AlertDialogTitle>
            <AlertDialogDescription>
              {t("runtimes.detail.deleteDescription", { name: runtime.name })}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>{t("runtimes.detail.cancel")}</AlertDialogCancel>
            <AlertDialogAction
              variant="destructive"
              onClick={handleDelete}
              disabled={deleteMutation.isPending}
            >
              {deleteMutation.isPending ? t("runtimes.detail.deleting") : t("runtimes.detail.delete")}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
