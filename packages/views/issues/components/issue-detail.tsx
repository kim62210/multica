...
function formatActivity(
  ...
  switch (entry.action) {
    case "created":
      return t("issueDetail.activity.created");
    case "status_changed":
      return t("issueDetail.activity.statusChanged", {
        from: statusLabel(details.from ?? "?"),
        to: statusLabel(details.to ?? "?"),
      });
    case "priority_changed":
      return t("issueDetail.activity.priorityChanged", {
        from: priorityLabel(details.from ?? "?"),
        to: priorityLabel(details.to ?? "?"),
      });
    case "assignee_changed": {
      const isSelfAssign = details.to_type === entry.actor_type && details.to_id === entry.actor_id;
      if (isSelfAssign) return t("issueDetail.activity.selfAssigned");
      const toName = details.to_id && details.to_type && resolveActorName
        ? resolveActorName(details.to_type, details.to_id)
        : null;
      if (toName) return t("issueDetail.activity.assignedTo", { name: toName });
      if (details.from_id && !details.to_id) return t("issueDetail.activity.removedAssignee");
      return t("issueDetail.activity.changedAssignee");
    }
    case "due_date_changed": {
      if (!details.to) return t("issueDetail.activity.removedDueDate");
      const formatted = new Date(details.to).toLocaleDateString("en-US", { month: "short", day: "numeric" });
      return t("issueDetail.activity.setDueDate", { date: formatted });
    }
    case "title_changed":
      return t("issueDetail.activity.titleChanged", {
        from: details.from ?? "?",
        to: details.to ?? "?",
      });
    case "description_updated":
      return t("issueDetail.activity.descriptionUpdated");
    case "task_completed":
      return t("issueDetail.activity.taskCompleted");
    case "task_failed":
      return t("issueDetail.activity.taskFailed");
    default:
      return entry.action ?? "";
  }
}
  ...
  return (
    ...
    >
      <Command shouldFilter={false}>
        <CommandInput
          placeholder={t("issueDetail.picker.searchIssues")}
          value={query}
          onValueChange={(v) => {
            setQuery(v);
            search(v);
        ...
        <CommandList>
          {isLoading && (
            <div className="py-6 text-center text-sm text-muted-foreground">
              {t("issueDetail.picker.searching")}
            </div>
          )}
          {!isLoading && query.trim() && results.length === 0 && (
            <CommandEmpty>{t("issueDetail.picker.noIssuesFound")}</CommandEmpty>
          )}
          {!isLoading && !query.trim() && (
            <div className="py-6 text-center text-sm text-muted-foreground">
              {t("issueDetail.picker.typeToSearch")}
            </div>
          )}
          {results.length > 0 && (
            <CommandGroup>
...
export function IssueDetail({ issueId, onDelete, defaultSidebarOpen = true, layoutId = "multica_issue_detail_layout", highlightCommentId }: IssueDetailProps) {
  ...
  const handleUpdateField = useCallback(
    (updates: Partial<UpdateIssueRequest>) => {
      ...
      updateIssueMutation.mutate(
        { id, ...updates },
        { onError: () => toast.error(t("issueDetail.update.error")) },
      );
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast.success(t("issueDetail.delete.success"));
      if (onDelete) onDelete();
      else router.push(paths.issues());
    } catch {
      toast.error(t("issueDetail.delete.error"));
      setDeleting(false);
    }
  };
    ...
    <div className="space-y-5">
      ...
      <div>
        ...
        {propertiesOpen && <div className="space-y-0.5 pl-2">
          <PropRow label={t("issueDetail.properties.status")}>
            <StatusPicker status={issue.status} onUpdate={handleUpdateField} align="start" />
          </PropRow>
          <PropRow label={t("issueDetail.properties.priority")}>
            <PriorityPicker priority={issue.priority} onUpdate={handleUpdateField} align="start" />
          </PropRow>
          <PropRow label={t("issueDetail.properties.assignee")}>
            <AssigneePicker assigneeType={issue.assignee_type} assigneeId={issue.assignee_id} onUpdate={handleUpdateField} align="start" />
          </PropRow>
          <PropRow label={t("issueDetail.properties.dueDate")}>
            <DueDatePicker dueDate={issue.due_date} onUpdate={handleUpdateField} />
          </PropRow>
          <PropRow label={t("issueDetail.properties.project")}>
      ...
      <div>
        ...
        {detailsOpen && <div className="space-y-0.5 pl-2">
          <PropRow label={t("issueDetail.details.createdBy")}>
          ...
          <PropRow label={t("issueDetail.details.created")}>
            <span className="text-muted-foreground">{shortDate(issue.created_at)}</span>
          </PropRow>
          <PropRow label={t("issueDetail.details.updated")}>
      ...
      {usage && usage.task_count > 0 && (
        <div>
          ...
          {tokenUsageOpen && <div className="space-y-0.5 pl-2">
            <PropRow label={t("issueDetail.tokenUsage.input")}>
              <span className="text-muted-foreground">{formatTokenCount(usage.total_input_tokens)}</span>
            </PropRow>
            <PropRow label={t("issueDetail.tokenUsage.output")}>
            ...
            {(usage.total_cache_read_tokens > 0 || usage.total_cache_write_tokens > 0) && (
              <PropRow label={t("issueDetail.tokenUsage.cache")}>
                <span className="text-muted-foreground">
                  {t("issueDetail.tokenUsage.cacheValue", {
            ...
            <PropRow label={t("issueDetail.tokenUsage.runs")}>
      ...
      <div className="flex h-full flex-col">
        <PageHeader className="gap-2 bg-background text-sm">
          ...
          <div className="flex items-center gap-1 shrink-0">
            <Tooltip>
              ...
              />
              <TooltipContent side="bottom">{isPinned ? t("issueDetail.header.unpinFromSidebar") : t("issueDetail.header.pinToSidebar")}</TooltipContent>
            </Tooltip>
            <DropdownMenu>
              <DropdownMenuTrigger
                render={
              ...
              <DropdownMenuContent align="end" className="w-auto">
                ...
                <DropdownMenuItem onClick={() => {
                  ...
                  {isPinned ? t("issueDetail.header.unpinFromSidebar") : t("issueDetail.header.pinToSidebar")}
                </DropdownMenuItem>
                ...
                <DropdownMenuItem onClick={() => {
                  ...
                  toast.success(t("issueDetail.menu.linkCopied"));
                }}>
                  <Link2 className="h-3.5 w-3.5" />
                  {t("issueDetail.menu.copyLink")}
                </DropdownMenuItem>
                ...
                >
                  ...
                  {t("issueDetail.menu.deleteIssue")}
                </DropdownMenuItem>
              </DropdownMenuContent>
            </DropdownMenu>
            <Tooltip>
              ...
              />
              <TooltipContent side="bottom">{t("issueDetail.header.toggleSidebar")}</TooltipContent>
            </Tooltip>
          </div>
        </PageHeader>
            ...
            <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
              <AlertDialogContent>
                <AlertDialogHeader>
                  <AlertDialogTitle>{t("issueDetail.delete.title")}</AlertDialogTitle>
                  <AlertDialogDescription>
                    {t("issueDetail.delete.description")}
                  </AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter>
                  <AlertDialogCancel>{t("issueDetail.delete.cancel")}</AlertDialogCancel>
                  <AlertDialogAction
                    onClick={handleDelete}
                    disabled={deleting}
                    className="bg-destructive text-white hover:bg-destructive/90"
                  >
                    {deleting ? t("issueDetail.delete.deleting") : t("issueDetail.delete.confirm")}
                  </AlertDialogAction>
                </AlertDialogFooter>
              </AlertDialogContent>
            </AlertDialog>
            ...
            <BacklogAgentHintDialog
              ...
              onMoveToTodo={() => {
                updateIssueMutation.mutate(
                  { id, status: "todo" },
                  { onError: () => toast.error(t("issueDetail.update.statusError")) },
                );
            ...
            <IssuePickerDialog
              ...
              title={t("issueDetail.picker.setParentTitle")}
              description={t("issueDetail.picker.setParentDescription")}
              excludeIds={[id, ...childIssues.map((c) => c.id)]}
              onSelect={(selected) => {
                handleUpdateField({ parent_issue_id: selected.id });
                toast.success(t("issueDetail.picker.setParentToast", { identifier: selected.identifier }));
              }}
            />
            ...
            <IssuePickerDialog
              ...
              title={t("issueDetail.picker.addSubTitle")}
              description={t("issueDetail.picker.addSubDescription")}
              excludeIds={[id, ...(parentIssueId ? [parentIssueId] : []), ...childIssues.map((c) => c.id)]}
              onSelect={(selected) => {
                updateIssueMutation.mutate(
                  { id: selected.id, parent_issue_id: id },
                  { onError: () => toast.error(t("issueDetail.picker.addSubError")) },
                );
                toast.success(t("issueDetail.picker.addedSubToast", { identifier: selected.identifier }));
              }}
            />
        ...
        <div className="mx-auto w-full max-w-4xl px-8 py-8">
          <TitleEditor
            ...
            placeholder={t("issueDetail.title.placeholder")}
          ...
          {parentIssue && (
            ...
            >
              <span className="font-medium shrink-0">{t("issueDetail.subIssueOf")}</span>
              <StatusIcon status={parentIssue.status} className="h-3.5 w-3.5 shrink-0" />
                    ...
                    <TooltipTrigger
                      render={
                        <button
                          ...
                          aria-label={t("issueDetail.addSubIssueLabel")}
                        >
                          <Plus className="h-4 w-4" />
                        </button>
                      }
                    />
                    <TooltipContent side="bottom">{t("issueDetail.addSubIssueLabel")}</TooltipContent>
                  </Tooltip>
                </div>
          ...
          <div>
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <h2 className="text-base font-semibold">{t("issueDetail.activityHeading")}</h2>
              </div>
              <div className="flex items-center gap-2">
                ...
                <Popover>
                  ...
                  <PopoverContent align="end" className="w-64 p-0">
                    <Command>
                      <CommandInput placeholder={t("issueDetail.subscribers.placeholder")} />
                      <CommandList className="max-h-64">
                        <CommandEmpty>{t("issueDetail.subscribers.noResults")}</CommandEmpty>
                        {members.length > 0 && (
                          <CommandGroup heading={t("issueDetail.subscribers.members")}>
                        ...
                        {agents.filter((a) => !a.archived_at).length > 0 && (
                          <CommandGroup heading={t("issueDetail.subscribers.agents")}>
