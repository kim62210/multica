    ...
    <>
      ...
      <div className="mt-4 sticky top-4 z-10">
        <SingleAgentLiveCard
          ...
          issueId={issueId}
          agentName={firstEntry.task.agent_id ? getActorName("agent", firstEntry.task.agent_id) : t("issueDetail.agent.agentFallback")}
        />
      </div>
        ...
        <div className="mt-1.5 space-y-1.5">
          {restEntries.map(({ task, items }) => (
            <SingleAgentLiveCard
              ...
              items={items}
              issueId={issueId}
              agentName={task.agent_id ? getActorName("agent", task.agent_id) : t("issueDetail.agent.agentFallback")}
            />
          ))}
        </div>
      )}
...
function SingleAgentLiveCard({ task, items, issueId, agentName }: SingleAgentLiveCardProps) {
  ...
  const handleCancel = useCallback(async () => {
    ...
    try {
      ...
      toast.error(e instanceof Error ? e.message : t("issueDetail.agent.cancelError"));
      setCancelling(false);
    }
    ...
    <div className="rounded-lg border border-info/20 bg-info/5 backdrop-blur-sm">
      ...
      >
        ...
        <div className="flex items-center gap-1.5 text-xs min-w-0">
          ...
          <span className="font-medium text-foreground truncate">{t("issueDetail.agent.isWorking", { name: agentName })}</span>
          <span className="text-muted-foreground tabular-nums shrink-0">{elapsed}</span>
          {toolCount > 0 && (
            <span className="text-muted-foreground shrink-0">{t("issueDetail.agent.tools", { count: toolCount })}</span>
          )}
        </div>
        <div className="ml-auto flex items-center gap-1 shrink-0">
          <button
            ...
            title={t("issueDetail.agent.expandTranscript")}
          >
  ...
  return (
    <Collapsible open={open} onOpenChange={setOpen}>
      <CollapsibleTrigger className="flex w-full items-center gap-1.5 text-xs text-muted-foreground hover:text-foreground transition-colors py-1">
        ...
        <span>{t("issueDetail.agent.executionHistory", { count: completedTasks.length })}</span>
      </CollapsibleTrigger>
      <CollapsibleContent>
        ...
        <div className="ml-5 mt-1 max-h-64 overflow-y-auto rounded border bg-muted/30 px-3 py-2 space-y-0.5">
          {items === null ? (
            <div className="flex items-center gap-2 text-xs text-muted-foreground py-2">
              ...
              {t("issueDetail.agent.loading")}
            </div>
          ) : items.length === 0 ? (
            <p className="text-xs text-muted-foreground py-2">{t("issueDetail.agent.noExecutionData")}</p>
          ) : (
            items.map((item, idx) => (
              <TimelineRow key={`${item.seq}-${idx}`} item={item} />
            ))
