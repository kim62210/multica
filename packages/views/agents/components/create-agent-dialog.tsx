...
export function CreateAgentDialog({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast.error(err instanceof Error ? err.message : t("agents.create.failed"));
      setCreating(false);
    }
  };
    ...
    <Dialog open onOpenChange={(v) => { if (!v) onClose(); }}>
      <DialogContent className="sm:max-w-md">
        ...
        <div className="space-y-4 min-w-0">
          <div>
            <Label className="text-xs text-muted-foreground">{t("agents.create.nameLabel")}</Label>
            <Input
              autoFocus
              type="text"
              value={name}
          ...
          <div>
            <Label className="text-xs text-muted-foreground">{t("agents.create.descriptionLabel")}</Label>
            <Input
              type="text"
              value={description}
          ...
          <div>
            <Label className="text-xs text-muted-foreground">{t("agents.create.visibilityLabel")}</Label>
            <div className="mt-1.5 flex gap-2">
              ...
              >
                ...
                <div className="text-left">
                  <div className="font-medium">{t("agents.create.visibilityWorkspace")}</div>
                  <div className="text-xs text-muted-foreground">{t("agents.create.visibilityWorkspaceHint")}</div>
                </div>
              </button>
              <button
                type="button"
              ...
              >
                ...
                <div className="text-left">
                  <div className="font-medium">{t("agents.create.visibilityPrivate")}</div>
                  <div className="text-xs text-muted-foreground">{t("agents.create.visibilityPrivateHint")}</div>
                </div>
              </button>
            </div>
          </div>
          ...
          <div className="min-w-0">
            <div className="flex items-center justify-between">
              <Label className="text-xs text-muted-foreground">{t("agents.create.runtimeLabel")}</Label>
              {hasOtherRuntimes && (
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    <span className="truncate font-medium">
                      {runtimesLoading ? t("agents.create.loadingRuntimes") : (selectedRuntime?.name ?? t("agents.create.noRuntimeAvailable"))}
                    </span>
                    {selectedRuntime?.runtime_mode === "cloud" && (
                      <span className="shrink-0 rounded bg-info/10 px-1.5 py-0.5 text-xs font-medium text-info">
                        {t("agents.create.runtimeCloud")}
                      </span>
                    )}
                  </div>
        ...
        <DialogFooter>
          ...
          >
            {creating ? t("agents.create.submitting") : t("agents.create.submit")}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
