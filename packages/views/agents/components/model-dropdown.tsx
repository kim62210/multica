...
export function ModelDropdown({
  ...
  const triggerLabel =
    value ||
    (disabled
      ? t("agents.model.selectRuntimeFirst")
      : runtimeOnline
        ? defaultModel
          ? t("agents.model.defaultNamed", { name: defaultModel.label })
          : t("agents.model.defaultProvider")
        : t("agents.model.offlineHint"));
  ...
  if (!supported && !modelsQuery.isLoading) {
    ...
    return (
      <div className="min-w-0">
        <Label className="text-xs text-muted-foreground">{t("agents.model.label")}</Label>
        <div className="mt-1.5 flex items-start gap-2 rounded-lg border border-dashed border-border bg-muted/30 px-3 py-2.5 text-sm text-muted-foreground">
          ...
          <div className="min-w-0">
            <div>{t("agents.model.unsupportedTitle")}</div>
            <div className="mt-0.5 text-xs">
              {t("agents.model.unsupportedHint")}
            </div>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="min-w-0">
      <div className="flex items-center justify-between">
        <Label className="text-xs text-muted-foreground">{t("agents.model.label")}</Label>
        {modelsQuery.isError && (
          <span className="text-xs text-muted-foreground">{t("agents.model.discoveryFailed")}</span>
        )}
      </div>
      <Popover open={open} onOpenChange={setOpen}>
        ...
        >
          <div className="border-b border-border p-2">
            <Input
              autoFocus
              placeholder={t("agents.model.searchPlaceholder")}
              value={search}
            ...
            {canCreate && (
              ...
              >
                ...
                <span className="truncate">
                  {t("agents.model.useCustom", { value: trimmedSearch })}
                </span>
              </button>
            )}
