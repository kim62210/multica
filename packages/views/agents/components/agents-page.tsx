...
export function AgentsPage() {
  ...
  const handleRestore = async (id: string) => {
    try {
      ...
      qc.invalidateQueries({ queryKey: workspaceKeys.agents(wsId) });
      toast.success(t("agents.restoreSuccess"));
    } catch (e) {
      ...
      <ResizablePanel id="list" defaultSize={280} minSize={240} maxSize={400} groupResizeBehavior="preserve-pixel-size">
        ...
        <div className="overflow-y-auto h-full border-r">
          <PageHeader className="justify-between">
            <h1 className="text-sm font-semibold">{t("agents.page.title")}</h1>
            <div className="flex items-center gap-1">
              {archivedCount > 0 && (
                <Button
                  ...
                  title={showArchived ? t("agents.page.showActive") : t("agents.page.showArchived")}
                >
          ...
          {filteredAgents.length === 0 ? (
            <div className="flex flex-col items-center justify-center px-4 py-12">
              ...
              <p className="mt-3 text-sm text-muted-foreground">
                {showArchived ? t("agents.page.emptyNoArchived") : archivedCount > 0 ? t("agents.page.emptyNoActive") : t("agents.page.emptyNoAgents")}
              </p>
              {!showArchived && (
                <Button
                  onClick={() => setShowCreate(true)}
      ...
      <ResizablePanel id="detail" minSize="50%">
        ...
        {selected ? (
          ...
          <div className="flex h-full flex-col items-center justify-center text-muted-foreground">
            ...
            <p className="mt-3 text-sm">{t("agents.page.selectToViewDetails")}</p>
            <Button
              onClick={() => setShowCreate(true)}
              size="xs"
              className="mt-3"
