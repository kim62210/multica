...
export function SettingsTab({
  ...
  const handleAvatarUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      toast.success(t("agents.settings.avatarUpdated"));
    } catch (err) {
      toast.error(err instanceof Error ? err.message : t("agents.settings.avatarUploadFailed"));
    }
  };
  ...
  const handleSave = async () => {
    if (!name.trim()) {
      toast.error(t("agents.settings.nameRequired"));
      return;
    }
    ...
    try {
      ...
      toast.success(t("agents.settings.saveSuccess"));
    } catch {
      toast.error(t("agents.settings.saveFailed"));
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="max-w-lg space-y-6">
      <div>
        <Label className="text-xs text-muted-foreground">{t("agents.settings.avatarLabel")}</Label>
        <div className="mt-1.5 flex items-center gap-4">
          ...
          <div className="text-xs text-muted-foreground">
            {t("agents.settings.avatarUploadHint")}
          </div>
        </div>
      </div>

      <div>
        <Label className="text-xs text-muted-foreground">{t("agents.settings.nameLabel")}</Label>
        <Input
          value={name}
      ...
      <div>
        <Label className="text-xs text-muted-foreground">{t("agents.settings.descriptionLabel")}</Label>
        <Input
          value={description}
      ...
      <div>
        <Label className="text-xs text-muted-foreground">{t("agents.settings.visibilityLabel")}</Label>
        <div className="mt-1.5 flex gap-2">
          ...
          >
            ...
            <div className="text-left">
              <div className="font-medium">{t("agents.settings.visibilityWorkspace")}</div>
              <div className="text-xs text-muted-foreground">{t("agents.settings.visibilityWorkspaceHint")}</div>
            </div>
          </button>
          <button
            type="button"
          ...
          >
            ...
            <div className="text-left">
              <div className="font-medium">{t("agents.settings.visibilityPrivate")}</div>
              <div className="text-xs text-muted-foreground">{t("agents.settings.visibilityPrivateHint")}</div>
            </div>
          </button>
        </div>
      </div>
      ...
      <div>
        <Label className="text-xs text-muted-foreground">{t("agents.settings.maxTasksLabel")}</Label>
        <Input
          type="number"
          min={1}
          max={50}
      ...
      <div>
        <div className="flex items-center justify-between">
          <Label className="text-xs text-muted-foreground">{t("agents.settings.runtimeLabel")}</Label>
          {hasOtherRuntimes && (
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-2">
                <span className="truncate font-medium">
                  {selectedRuntime?.name ?? t("agents.settings.noRuntimeAvailable")}
                </span>
                {selectedRuntime?.runtime_mode === "cloud" && (
                  <span className="shrink-0 rounded bg-info/10 px-1.5 py-0.5 text-xs font-medium text-info">
                    {t("agents.settings.runtimeCloud")}
                  </span>
                )}
              </div>
              <div className="truncate text-xs text-muted-foreground">
                ...
                ) : t("agents.settings.selectRuntime")}
              </div>
            </div>
