...
export function EnvTab({
  ...
  const handleSave = async () => {
    ...
    if (uniqueKeys.size < keys.length) {
      toast.error(t("agents.env.duplicateError"));
      return;
    }
    ...
    try {
      ...
      toast.success(t("agents.env.saveSuccess"));
    } catch {
      toast.error(t("agents.env.saveFailed"));
    } finally {
      setSaving(false);
    }
  };
      ...
      <div className="max-w-lg space-y-4">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-0.5">
            {t("agents.env.descriptionReadonly")}
          </p>
        </div>
        {envEntries.length > 0 ? (
          ...
          <p className="text-xs text-muted-foreground italic">{t("agents.env.emptyReadonly")}</p>
        )}
      </div>
    );
  }
