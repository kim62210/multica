...
export function InstructionsTab({
  ...
  return (
    <div className="space-y-4">
      <div>
        <h3 className="text-sm font-semibold">{t("agents.instructions.title")}</h3>
      ...
      <div className="flex items-center justify-between">
        <span className="text-xs text-muted-foreground">
          {value.length > 0 ? t("agents.instructions.charCount", { count: value.length }) : t("agents.instructions.empty")}
        </span>
        <Button
          size="xs"
          onClick={handleSave}
