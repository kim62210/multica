...
function HeadingDropdown({ editor, onOpenChange, activeLevel }: { editor: Editor; onOpenChange: (open: boolean) => void; activeLevel: number | undefined }) {
  ...
  const label = activeLevel ? `H${activeLevel}` : t("editor.heading.text");
  const items = [
    { label: t("editor.heading.normal"), icon: Type, active: !activeLevel, action: () => editor.chain().focus().setParagraph().run() },
    { label: t("editor.heading.h1"), icon: Heading1, active: activeLevel === 1, action: () => editor.chain().focus().toggleHeading({ level: 1 }).run() },
    { label: t("editor.heading.h2"), icon: Heading2, active: activeLevel === 2, action: () => editor.chain().focus().toggleHeading({ level: 2 }).run() },
    { label: t("editor.heading.h3"), icon: Heading3, active: activeLevel === 3, action: () => editor.chain().focus().toggleHeading({ level: 3 }).run() },
  ];
  ...
  return (
    <Popover modal={false} open={open} onOpenChange={handleOpenChange}>
      <Tooltip>
        ...
        <TooltipContent side="top" sideOffset={8}>{t("editor.list.tooltip")}</TooltipContent>
      </Tooltip>
      <PopoverContent
        side="bottom"
        sideOffset={8}
      ...
      >
        ...
        >
          <List className="size-3.5" /> {t("editor.list.bullet")}
        ...
        >
          <ListOrdered className="size-3.5" /> {t("editor.list.ordered")}
...
function CreateSubIssueButton({
  ...
  const handleClick = useCallback(async () => {
    ...
    try {
      ...
      editor
        ...
        .run();
      toast.success(t("editor.subIssue.createdToast", { identifier: newIssue.identifier }));
    } catch {
      toast.error(t("editor.subIssue.createFailedToast"));
    } finally {
      setPending(false);
    }
  ...
  return (
    <Tooltip>
      ...
      <TooltipContent side="top" sideOffset={8}>
        {t("editor.subIssue.buttonTooltip")}
      </TooltipContent>
    </Tooltip>
  );
}
      ...
      {mode === "link-edit" ? (
        ...
        <TooltipProvider delay={300}>
          <div className="bubble-menu">
            <MarkButton editor={editor} mark="bold" icon={Bold} label={t("editor.mark.bold")} shortcut={`${mod}+B`} isActive={fmt.bold} />
            <MarkButton editor={editor} mark="italic" icon={Italic} label={t("editor.mark.italic")} shortcut={`${mod}+I`} isActive={fmt.italic} />
            <MarkButton editor={editor} mark="strike" icon={Strikethrough} label={t("editor.mark.strike")} shortcut={`${mod}+Shift+S`} isActive={fmt.strike} />
            <MarkButton editor={editor} mark="code" icon={Code} label={t("editor.mark.code")} shortcut={`${mod}+E`} isActive={fmt.code} />
            ...
            <Tooltip>
              ...
              <TooltipContent side="top" sideOffset={8}>{t("editor.link.tooltip")}</TooltipContent>
            </Tooltip>
            <Separator orientation="vertical" className="mx-0.5 h-5" />
            ...
            <Tooltip>
              ...
              <TooltipContent side="top" sideOffset={8}>{t("editor.quote.tooltip")}</TooltipContent>
            </Tooltip>
            {currentIssueId && (
              <>
                <Separator orientation="vertical" className="mx-0.5 h-5" />
