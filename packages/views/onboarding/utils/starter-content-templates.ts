...
export function buildWelcomeIssueText(
  ...
  const sharedInstructions = [
    ...
    `2. **Explain how we work together in SWARM**:`,
  ...
  switch (q.use_case) {
    case "coding":
      return {
        title: "👋 Welcome to SWARM — let's work together",
        description: `${header}Hi agent, this is ${name}'s first time using SWARM. They plan to use you mostly for **coding work**.\n\n${sharedInstructions}`,
      };
    case "planning":
      return {
        title: "👋 Welcome to SWARM — let's work together",
        description: `${header}Hi agent, this is ${name}'s first time using SWARM. They want your help with **planning and breaking down work**.\n\n${sharedInstructions}`,
      };
    case "writing_research":
      return {
        title: "👋 Welcome to SWARM — let's work together",
        description: `${header}Hi agent, this is ${name}'s first time using SWARM. They'll use you for **research and writing** — drafting, summarizing, analysis.\n\n${sharedInstructions}`,
      };
    case "explore":
      return {
        title: "👋 Welcome to SWARM — let's work together",
        description: `${header}Hi agent, this is ${name}'s first time using SWARM. They're **exploring** what SWARM can do — no specific goal yet.\n\n${exploreInstructions}`,
      };
    case "other": {
      ...
      return {
        title: "👋 Welcome to SWARM — let's work together",
        description: `${header}Hi agent, this is ${name}'s first time using SWARM. ${contextLine}\n\n${sharedInstructions}`,
      };
    }
    default:
      return {
        title: "👋 Welcome to SWARM — let's work together",
        description: `${header}Hi agent, this is ${name}'s first time using SWARM.\n\n${sharedInstructions}`,
      };
  }
}
...
export function buildAgentGuidedSubIssues(
  ...
  const tier1: ImportStarterIssuePayload[] = [
    {
      ...
      description: [
        `**Every issue has a right-side panel** called **Properties**. From there you control who works on what. Agents in SWARM are triggered when an issue has:`,
        ``,
  ...
  if (q.team_size === "team") {
    tier2.push({
      ...
      description: [
        `SWARM works best when a small team shares agents.`,
        ``,
        `**Where to find it**:`,
...
export function buildSelfServeSubIssues(
  ...
  // Without a runtime + an agent, nothing else in SWARM works. These two
...
export function buildImportPayload({
  ...
  return {
    ...
    project: {
      ...
      description:
        "A few things to try in SWARM. Work through them at your own pace.",
      icon: "👋",
    },
