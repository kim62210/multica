...
export function CreateProjectModal({ onClose }: { onClose: () => void }) {
  ...
  const leadLabel = leadType && leadId ? getActorName(leadType, leadId) : t("modals.createProject.lead.placeholder");
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      onClose();
      toast.success(t("modals.createProject.toast.created"));
      router.push(wsPaths.projectDetail(project.id));
    } catch {
      toast.error(t("modals.createProject.error.create"));
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <Dialog open onOpenChange={(v) => { if (!v) onClose(); }}>
      ...
      >
        <DialogTitle className="sr-only">{t("modals.createProject.srTitle")}</DialogTitle>

        <div className="flex items-center justify-between px-5 pt-3 pb-2 shrink-0">
          <div className="flex items-center gap-1.5 text-xs">
            <span className="text-muted-foreground">{workspaceName}</span>
            <ChevronRight className="size-3 text-muted-foreground/50" />
            <span className="font-medium">{t("modals.createProject.breadcrumb")}</span>
          </div>
            ...
            <PopoverTrigger
              render={
                <button
                  ...
                  title={t("modals.createProject.iconPickerTitle")}
                >
                  {icon || "📁"}
                </button>
              }
              ...
              render={
                <PillButton>
                  {leadType && leadId ? (
                    ...
                    <span className="text-muted-foreground">{t("modals.createProject.lead.placeholder")}</span>
                  )}
                </PillButton>
              }
            />
            <PopoverContent align="start" className="w-52 p-0">
              <div className="px-2 py-1.5 border-b">
                <input
                  ...
                  placeholder={t("modals.createProject.lead.assignPlaceholder")}
              ...
              <div className="p-1 max-h-60 overflow-y-auto">
                ...
                >
                  ...
                  <span className="text-muted-foreground">{t("modals.createProject.lead.noLead")}</span>
                </button>
                {filteredMembers.length > 0 && (
                  <>
                    <div className="px-2 pt-2 pb-1 text-xs font-medium text-muted-foreground uppercase tracking-wider">
                      {t("modals.createProject.lead.members")}
                    </div>
                ...
                {filteredAgents.length > 0 && (
                  <>
                    <div className="px-2 pt-2 pb-1 text-xs font-medium text-muted-foreground uppercase tracking-wider">
                      {t("modals.createProject.lead.agents")}
                    </div>
                ...
                {filteredMembers.length === 0 &&
                  ...
                  leadFilter && (
                    <div className="px-2 py-3 text-center text-sm text-muted-foreground">
                      {t("modals.createProject.lead.noResults")}
                    </div>
                  )}
              </div>
            </PopoverContent>
