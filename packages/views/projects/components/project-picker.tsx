  ...
  return (
    <DropdownMenu>
      ...
      >
        ...
        <span className="truncate">{current ? current.title : t("projects.picker.noProject")}</span>
      </DropdownMenuTrigger>
      <DropdownMenuContent align={align} className="w-52">
        ...
        {projectId && (
          <DropdownMenuItem onClick={() => onUpdate({ project_id: null })}>
            ...
            {t("projects.picker.remove")}
          </DropdownMenuItem>
        )}
        {projects.length === 0 && (
          <div className="px-2 py-1.5 text-xs text-muted-foreground">{t("projects.picker.empty")}</div>
        )}
      </DropdownMenuContent>
    </DropdownMenu>
  );
